...
exports.addQuestionToSet = (setID, questionID) => {
    ...
    return new Promise((resolve, reject) => {
        db.query(
            "Select * from setquestionentry where setID = ? and questionID = ?",
            [setID, questionID],
            (err, results) => {
                if (err) {
                ...
                if (results.length > 0) {
                    ...
                    db.query(
                        "insert into setquestionentry (setID, questionID) values (?, ?)",[setID, questionID],
                        (err, result) => {
                            if (err) {
...
exports.removeQuestionFromSet = (setID, questionID) => {
    return new Promise((resolve, reject) => {
        db.query(
            "delete from setquestionentry where setID = ? AND questionID = ?",
            [setID, questionID],
            (err, result) => {
                if (err) {
                    reject(err.sqlMessage || "database error");
